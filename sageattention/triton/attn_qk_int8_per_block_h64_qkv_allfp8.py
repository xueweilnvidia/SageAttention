...
def _attn_fwd_inner(acc, l_i, m_i, q, q_scale, 
    ...
    for start_n in range(lo, hi, BLOCK_N):
        ...
        middle = tl.dot(p_int8, v, accumulator).to(tl.float32) * v_scale *p_scale
        acc = acc + middle  
        m_i = m_ij
...
@triton.jit
def _attn_fwd(Q, K, V, Q_scale, K_scale, V_scale, Out,  
    ...
    v_scale_offset = off_hz * tl.cdiv(N_CTX, BLOCK_K)##
    ...
    acc, l_i = _attn_fwd_inner(acc, l_i, m_i, q, q_scale, K_ptrs, K_scale_ptr, V_ptrs, V_scale_ptr,  
                                    start_m,  
...
def forward(q, k, v, q_scale, k_scale, v_scale):
    ...
    _attn_fwd[grid](
        q, k, v, q_scale, k_scale, v_scale, o,  
